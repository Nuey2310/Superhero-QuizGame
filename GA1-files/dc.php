
<!-- contributed by:
    Name: Ishan Bhatia
    Banner Number: B00835259
    - Implemented the front end aspects of the game
    - Implemented the loop for displaying the questions 

     Neuer Gao
    Banner: B00785904
    - Making questions can be dynamically displayed on the webpage
    - Making Options can be dynamically displayed on the webpage
    - Making div Ids and Questions numbers, from names, from Ids generated by the script automatically.

    Arjun Banga
    Banner: B00852696
    - Implementing the quiz timer using JS.
    - Handling the questions and options content created above to use in checkAnswers script

    Miftahul Kashfy
    Banner: B00850212
    - Fixed the sumbit button for the dc page, was not working-->
    

<?php
    include "includes/header.php";
?>

<div class="landingImg2">
    <div class="heads">
        
        <p id="allPageHeading">DC</p>
    </div>
</div>

<p id="quizIntroPara"> Are you a True DC fan? Let's see how many of these questions you can get correct! <br> Finish the quiz before the time runs out! </p>

<main class="marvelBody">

    <div class="mainContent">

        <div class="sideBar">

            <p id="quizHeading1">DC QUIZ</p>

            <div class="questionContainer">

                <a href="#questionNumber1" class="questions1">Question 1</a>
                <a href="#questionNumber2" class="questions1">Question 2</a>
                <a href="#questionNumber3" class="questions1">Question 3</a>
                <a href="#questionNumber4" class="questions1">Question 4</a>
                <a href="#questionNumber5" class="questions1">Question 5</a>
                <a href="#questionNumber6" class="questions1">Question 6</a>
                <a href="#questionNumber7" class="questions1">Question 7</a>
                <a href="#questionNumber8" class="questions1">Question 8</a>
                <a href="#questionNumber9" class="questions1">Question 9</a>
                <a href="#questionNumber10" class="questions1">Question 10</a>
            </div>

            <div id="submitButtonContainer">

                <input type="submit" value="Submit" class="submitButton1" form = "quizForm">
                
            </div>

        </div>
<!--        Get the data from the database-->

        <div class="questionContent">

            <div id = "timer" style = "position: sticky; top: 2%;">
                    <h1 id = "timer-content" style = "float: right; margin-right: 1%; font-family: 'Roboto', 'Helvetica', sans-serif; font-size: 1.8vw;">
                    <span> Timer: </span>
                    <span id = "time">00:05:00</span>
                    </h1>
            </div>
            //                Use POST method to send form by default.
        <form action="includes/checkAnswers.php" method = "POST" id = "quizForm">
        <?php
            require_once "includes/db.php";
            $query = "SELECT*FROM `question-data` WHERE `Type` = 'DC'";
            $result = $dbConnection->query($query);
            $x = 1;
//            Use wile loop to get data from the database
            while ($row = $result->fetch_assoc()) {
            
        ?>

                <div class="individualQuestions">

                    <?php
//                  These variables are used to diplay questionString and options in html
                    $questionString = $row['questionText'];
                    $optionA = $row['option1'];
                    $optionB = $row['option2'];
                    $optionC = $row['option3'];
                    $correct = $row['correctOption'];

                    $hereDoc = <<<END
                    
                    <div id="questionNumber$x">

                        <p class="questionText">Question $x:<br><br>$questionString</p>

                    </div>
                    
                        <label for="choiceA$x"> <div class = "options1">
                            <input type="radio" name = "selection$x" id = "choiceA$x" value = "A">
                            $optionA
                        </div></label>

                        <br>

                        <label for="choiceB$x"> <div class = "options1">
                            <input type="radio" name = "selection$x" id = "choiceB$x" value = "B">
                            $optionB
                        </div></label>

                        <br>

                        <label for="choiceC$x"> <div class = "options1">
                            <input type="radio" name = "selection$x" id = "choiceC$x" value = "C">
                            $optionC
                        </div></label>
                        <input type="hidden" name = "correct$x" value="$correct"/> 
                        
                    END;

                    echo $hereDoc;
                    ?>

                </div>
                <?php
                    $x++;
                    }
                ?>
            </form>
        </div>

    </div>

</main>

<?php
    include "includes/footer.php";
?>





